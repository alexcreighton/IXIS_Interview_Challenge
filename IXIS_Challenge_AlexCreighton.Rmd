---
title: "IXIS_Challenge_AlexCreighton"
author: "Alex Creighton"
date: "7/12/2021"
output: pdf_document
---

This file is for analyzing client data and evaluating performance. Detailed instructions are included within each section. 
...
2 - Part 1
  - Necessary clean-up and calculations for sheet 1 of output file.

3 - Part 2
  3a - Initial clean-up and setup for calculations for sheet 2.
  3b - Creating final dataset and calculations.
  3c - Plots.
  
4 - Part 3
  - Creating the output file with sheet 1 and sheet 2 of final task.
  
5 - Part 4
  - Additional code for plots and methods of analysis that could be used.


# Section 1 - Setup

This section initializes necessary libraries and calls the desired datasets to be analyzed.
...
Instructions: "The first sheet should contain a Month * Device aggregation of the data with the following metrics: Sessions, Transactions, QTY, and ECR (= Transactions / Sessions)." 

###Sheet 1
This section performs necessary clean-up and calculations required for sheet 1 of the file that will be created.
...
month_by_device <- data.frame(sessionCounts$month, sessionCounts$device, sessionCounts$sessions, sessionCounts$transactions, sessionCounts$QTY, sessionCounts$ECR) #add necessary columns to new dataset for sheet 1 
...
Instructions: "The second sheet should contain a Month over Month comparison (for the most recent two months in the data) for all available metrics (including Adds to Cart), showing: the most recent month’s value, the prior month’s value, and both the absolute and relative differences between them."

###Sheet 2
###3a
This section performs initial clean-up and creates new datasets required for calculations to be used for sheet 2 of the file that will be created.
...
###3b
This section creates the dataset with month over month calculations and displays the figures that we are calculating.
...
month_over_month <- as.data.frame(t(month_over_month))
```

###3c
This section provides plots to analyze the performance visually.
...
Instructions: "Write an R script that produces an xlsx file with two worksheets, which would serve as reference tables delivered with the slide deck."
...
This section includes additional code that allows for insight into the data and performance.
